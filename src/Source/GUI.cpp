...
void GUI::renderAimbotWindow(Config& config, bool contentOnly) noexcept
{
    ...
    ImGui::Checkbox("Snap", &config.aimbot[currentWeapon].aimlock);
    ImGui::Checkbox("Legit", &config.aimbot[currentWeapon].silent);
    ...
    ImGui::Checkbox("Through flash", &config.aimbot[currentWeapon].ignoreFlash);
    ImGui::Checkbox("Through smoke", &config.aimbot[currentWeapon].ignoreSmoke);
...
void GUI::renderStyleWindow(Config& config, bool contentOnly) noexcept
{
    ...
    if (ImGui::Combo("Menu style", &config.style.menuStyle, "RoxWare\0SelectFire\0"))
        window = { };
    if (ImGui::Combo("Menu colors", &config.style.menuColors, "Dark\0Light\0Purple\0Custom\0"))
        updateColors(config);
    ImGui::PopItemWidth();
    ...
    if (ImGui::ListBox("", &currentConfig, [](void* data, int idx, const char** out_text) {
        ...
        *out_text = (const char*)vector[idx].c_str();
        ...
        if (ImGui::BeginPopup("Config to reset")) {
            static constexpr const char* names[]{ "Whole", "Aimbot", "Trigbot", "Backtrack", "AA", "Glow", "ESP", "Visuals", "Inv Edit", "Sounds", "Style", "Misc" };
