...
void Misc::watermark(const Memory& memory) noexcept
{
    ...
    ImGui::Text("RoxWare | %d fps | %d ms", frameRate != 0.0f ? static_cast<int>(1 / frameRate) : 0, GameData::getNetOutgoingLatency());
    ImGui::End();
}
...
void Misc::drawGUI(const Engine& engine, const ClientInterfaces& clientInterfaces, const Interfaces& interfaces, const Memory& memory, bool contentOnly) noexcept
{
    ...
    ImGui::Checkbox("Strafer", &miscConfig.autoStrafe);
    ImGui::Checkbox("Bhop", &miscConfig.bunnyHop);
    ImGui::Checkbox("Fast duck", &miscConfig.fastDuck);
    ImGui::Checkbox("Slidewalk", &miscConfig.moonwalk);
    ...
    ImGui::Checkbox("Show Money", &miscConfig.revealMoney);
    ImGui::Checkbox("Reveal suspect", &miscConfig.revealSuspect);
    ImGui::Checkbox("Show Votes", &miscConfig.revealVotes);

    ImGui::Checkbox("Spectators", &miscConfig.spectatorList.enabled);
    ImGui::SameLine();
    ...
    if (ImGui::BeginPopup("")) {
        ImGui::Checkbox("No Title", &miscConfig.spectatorList.noTitleBar);
        ImGui::EndPopup();
    }
    ImGui::PopID();
    ...
    */
    ...
    ImGui::Checkbox("Left Hand Knife", &miscConfig.oppositeHandKnife);
